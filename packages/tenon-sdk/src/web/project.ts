...
export class TenonSDKProject {
  ...
  async getProjectInfoFromRemote() {
    const res = await (await fetch(`${this.app.config.mode === 'prod' ? '' : 'http://localhost:9847/'}getSDKProjectInfo?projectId=${this.app.page.pageInfo.value.belongProjectId}`)).json();
